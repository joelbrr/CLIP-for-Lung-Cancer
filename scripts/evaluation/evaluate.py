...
def visualize_embeddings(image_embeddings, text_embeddings, patient_ids):
    # Concatenamos las embeddings de imágenes y textos
    all_embeddings = np.concatenate([image_embeddings, text_embeddings], axis=0)
    
    # Reducción de dimensionalidad previa con PCA
    pca = PCA(n_components=10)  # Reducimos las dimensiones a 10
    reduced_embeddings = pca.fit_transform(all_embeddings)
    
    # Aplicamos t-SNE para reducir a 2D
    tsne = TSNE(n_components=2, random_state=0, n_iter=300)  # Reducimos las iteraciones de t-SNE
    embeddings_2d = tsne.fit_transform(reduced_embeddings)
    
    # Visualización con los IDs de pacientes al lado de cada punto
    plt.figure(figsize=(10, 8))
    for i, patient_id in enumerate(patient_ids):
        # Dibujamos las imágenes
        plt.scatter(embeddings_2d[i, 0], embeddings_2d[i, 1], c='b', label="Images" if i == 0 else "")
        plt.text(embeddings_2d[i, 0], embeddings_2d[i, 1], patient_id, fontsize=9, ha='right')

    for i, patient_id in enumerate(patient_ids):
        # Dibujamos los textos
        plt.scatter(embeddings_2d[len(image_embeddings) + i, 0], embeddings_2d[len(image_embeddings) + i, 1], c='r', label="Texts" if i == 0 else "")
        plt.text(embeddings_2d[len(image_embeddings) + i, 0], embeddings_2d[len(image_embeddings) + i, 1], patient_id, fontsize=9, ha='left')

    plt.legend()
    plt.title("2D Projection of Image and Text Embeddings (with Patient IDs)")
